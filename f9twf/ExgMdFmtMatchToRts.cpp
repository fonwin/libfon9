...
f9twf_API void I024MatchParserToRts(ExgMcMessage& e) {
   ...
   *parser.Rts_.AllocPacket<uint8_t>() = fon9::cast_to_underlying(parser.Flags_);
   if (IsEnumContains(e.Channel_.GetChannelMgr()->Symbs_->CtrlFlags_, fon9::fmkt::MdSymbsCtrlFlag::HasChannelSeq))
      fon9::ToBitv(parser.Rts_, symb.Deal_.Data_.ChannelSeq_ = pk.GetChannelSeq());
   f9fmkt::PackMktSeq(parser.Rts_, symb.Deal_.Data_.MarketSeq_, e.Channel_.GetChannelMgr()->Symbs_->CtrlFlags_, pk.ProdMsgSeq_);
